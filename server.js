...
// ---------------- Dome Phone (public/private + Gartic-style game) ----------------
const rooms = new Map();
...
function snap(r){ return { code:r.code, size:r.size, hostId:r.hostId, stage:r.stage, round:r.round, members:[...r.members.entries()].map(([id,m])=>({id,name:m.name,isHost:m.isHost})) }; }
...
io.of('/').on('connection', (socket)=>{
  ...
  socket.on('dp:create', ({name='Guest',size=8}={})=>{
    const code=makeCode(6); const r=ensureRoom(code);
    r.size=Math.min(12,Math.max(3,parseInt(size,10)||8));
    r.hostId=socket.id; r.stage='lobby'; r.round=0; r.order=[]; r.assignments={}; r.submissions={}; r.chains=[]; r.members.clear();
    r.members.set(socket.id,{name:String(name).slice(0,16),isHost:true});
    socket.join('dp:'+code); socket.data.dpRoom=code; emitRoom(r);
  });

  socket.on('dp:join', ({name='Guest',code}={})=>{
    const r=roomBy(String(code||'').toUpperCase()); if(!r) return socket.emit('dp:status','Room not found');
    ...
    socket.join('dp:'+r.code); socket.data.dpRoom=r.code; emitRoom(r);
  });
  ...
  socket.on('dp:public', ({name='Guest',size=8}={})=>{
    ...
    for(const r of rooms.values()){
      if(r.size===target && r.stage==='lobby' && r.members.size<r.size){
        r.members.set(socket.id,{name:String(name).slice(0,16),isHost:false});
        socket.join('dp:'+r.code); socket.data.dpRoom=r.code; emitRoom(r); return;
      }
    }
    const code=makeCode(6); const r=ensureRoom(code);
    r.size=target; if(!r.hostId) r.hostId=socket.id;
  ...
  socket.on('game:start', ()=>{
    const r=inRoom(); if(!r) return; if(socket.id!==r.hostId) return;
    if(r.members.size<3){ io.to('dp:'+r.code).emit('dp:status','Need at least 3 players'); return; }
    r.stage='prompt'; r.round=0; r.order=[...r.members.keys()]; r.assignments={}; r.submissions={}; r.chains=r.order.map(id=>({owner:r.members.get(id).name,sequence:[]}));
  ...
  function emitState(r){
    ...
    for(const [id,_m] of r.members){
      const st={...base};
      if(r.stage==='draw'){
        const idx=r.order.indexOf(id); const fromIdx=((r.round+idx)%r.order.length); const ownerId=r.order[fromIdx];
        const prompt=r.submissions.prompt?.get(ownerId); st.assignment=(prompt&&prompt.text)||'Draw anything!';
      }
      if(r.stage==='guess'){
        const idx=r.order.indexOf(id); const fromIdx=((r.round+idx)%r.order.length); const ownerId=r.order[fromIdx];
        const draw=r.submissions.draw?.get(ownerId); st.assignmentImage=draw?draw.image:null;
      }
      io.to(id).emit('game:state',st);
    }
  }
  function startTimer(r,sec){
    clearTimer(r); r._timerCount=sec;
    io.to('dp:'+r.code).emit('game:timer',r._timerCount);
    r._timer=setInterval(()=>{
      r._timerCount--; io.to('dp:'+r.code).emit('game:timer',r._timerCount);
      if(r._timerCount<=0){ clearTimer(r); checkAdvance(r,true); }
    },1000);
  }
    ...
    if(subs>=total||timeout){
      ...
      if(r.stage==='guess'){
        ...
        for(let i=0;i<ids.length;i++){
          const ownerId=ids[i]; const prompt=r.submissions.prompt?.get(ownerId); const draw=r.submissions.draw?.get(ownerId); const guess=r.submissions.guess?.get(ownerId);
          const chain=r.chains[i]; if(prompt && chain.sequence.length===0) chain.sequence.push({type:'text',value:prompt.text||''});
          if(draw) chain.sequence.push({type:'image',value:draw.image}); if(guess) chain.sequence.push({type:'text',value:guess.text||''});
        }
        r.round+=1;
        if(r.round>=r.members.size){
          r.stage='results'; clearTimer(r);
          io.to('dp:'+r.code).emit('game:results',{stories:r.chains.map(c=>({owner:c.owner,sequence:c.sequence}))}); emitState(r);
        } else {
          r.submissions={}; r.stage='prompt'; emitState(r); startTimer(r,50);
        }
      }
    }
  }
  ...
  socket.on('disconnect', ()=>{
    const code=socket.data.dpRoom; if(!code) return; const r=rooms.get(code); if(!r) return;
    r.members.delete(socket.id);
    if(socket.id===r.hostId){ const first=r.members.keys().next().value; r.hostId=first||null; if(first&&r.members.get(first)) r.members.get(first).isHost=true; }
    emitRoom(r); if(r.members.size===0) rooms.delete(code);
  });
});

const PORT = process.env.PORT || 3000;
server.listen(PORT, ()=> console.log('✅ DomeChat + Dome Phone listening on', PORT));
