...
app.get('/ice', async (_req,res)=>{
  ...
  try {
    ...
    else if (process.env.ICE_SERVERS_JSON) cachedIce = JSON.parse(process.env.ICE_SERVERS_JSON);
    else cachedIce = DEFAULT_ICE;
  } catch {
    try { cachedIce = process.env.ICE_SERVERS_JSON ? JSON.parse(process.env.ICE_SERVERS_JSON) : DEFAULT_ICE; }
    catch { cachedIce = DEFAULT_ICE; }
  }
...
function ipHash(req){const raw=(req.headers['x-forwarded-for']||req.socket.remoteAddress||'')+'|pepper';try{return crypto.createHash('sha256').update(raw).digest('hex').slice(0,16)}catch{return'unknown'}}
...
io.on('connection',(socket)=>{
  ...
  socket.on('reaction',({emoji}={})=>{
    if(rl(reactionLimiter,socket.id,500)) return; // rate-limit
  ...
  socket.on('report',(payload={})=>{
    ...
    const rec={reporter:socket.id, reported:partnerId, topic:String(topic||''), reason:'Nudity/sexual content', ts:now(), ipHash:'anon:'+ipHash(socket.request)};
    reports.push(rec);
